...
- **Automatic Lossless Conversion**: When the bot downloads a file, it checks the actual audio codec using `ffprobe`. If the file is lossless (WAV, FLAC, AIFF), it is immediately converted to MP3 320 CBR via `ffmpeg` (retaining metadata). The original heavy file is deleted, and the rest of the bot pipeline (including AI analysis and upload) exclusively handles the MP3. This ensures the AI sees the final MP3 stats (reducing size discrepancies) and drastically cuts bandwidth usage during upload.
